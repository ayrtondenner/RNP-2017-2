...
import Dataset_Reader.read_DeepBuildingDataset as DeepBuildingDataset
from six.moves import xrange


class GAN(object):
    def __init__(self, z_dim, batch_size, crop_image_size, resized_image_size, data_dir):
        deepBuilding_dataset = DeepBuildingDataset.read_dataset(data_dir)
        self.z_dim = z_dim
        #self.crop_image_size = crop_image_size
        #self.resized_image_size = resized_image_size
        self.batch_size = batch_size
        filename_queue = tf.train.string_input_producer(deepBuilding_dataset.train_images)
    ...
    def _read_input(self, filename_queue):
        ...
        decoded_image = tf.image.decode_jpeg(value,
                                             channels=1)  # Assumption:Color images are read and are to be generated

        decoded_image_4d = tf.expand_dims(decoded_image, 0)
        resized_image = tf.image.resize_bilinear(decoded_image_4d, [128, 64])
        record.input_image = tf.squeeze(resized_image, squeeze_dims=[0])

        # cropped_image = tf.cast(
        #     tf.image.resize_images(decoded_image, 55, 35, self.crop_image_size, self.crop_image_size),
        #     tf.float32)
    ...
    def _generator(self, z, dims, train_phase, activation=tf.nn.relu, scope_name="generator"):
        ...
        with tf.variable_scope(scope_name) as scope:
            ...
            h_bnz = tf.contrib.layers.batch_norm(inputs=h_z, decay=0.9, epsilon=1e-5, is_training=train_phase, scope="gen_bnz")
            ...
            for index in range(N - 2):
                ...
                h_bn = tf.contrib.layers.batch_norm(inputs=h_conv_t, decay=0.9, epsilon=1e-5, is_training=train_phase, scope="gen_bn%d" % index)
        ...
        with tf.variable_scope(scope_name) as scope:
            ...
            for index in range(N - 2):
                ...
                if skip_bn:
                    ...
                    #h_bn = utils.batch_norm(h_conv, dims[index + 1], train_phase, scope="disc_bn%d" % index, scope_reuse=scope_reuse)
