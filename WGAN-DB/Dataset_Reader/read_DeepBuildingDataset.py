...
def read_dataset(data_dir):
    pickle_filename = "deepBuilding.pickle"
    pickle_filepath = os.path.join(data_dir, pickle_filename)
    if not os.path.exists(pickle_filepath):
        # utils.maybe_download_and_extract(data_dir, DATA_URL, is_zipfile=True)
        deepBuilding_folder = 'resized'
        dir_path = os.path.join(data_dir, deepBuilding_folder)
        if not os.path.exists(dir_path):
            print ("DeepBuilding dataset needs to be downloaded and unzipped manually")
    ...
    with open(pickle_filepath, 'rb') as f:
        ...
        deepBuildingReader = DeepBuilding_Dataset(result)
        del result
    return deepBuildingReader
